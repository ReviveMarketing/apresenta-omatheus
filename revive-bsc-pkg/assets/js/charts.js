...
const Charts = {
  ...
  renderRocket(svgEl, dailyCounts, options = {}) {
    ...
    let svg = `
      ...
      <!-- X labels: usa labels customizados se fornecidos (modo acumulado mostra meses) -->
      ${dailyCounts.map((v, i) => {
        const labels = options.labels;
        let showLabel, labelText;
        if (labels) {
          showLabel = !!labels[i];
          labelText = labels[i] || '';
        } else {
          showLabel = i === 0 || i === days - 1 || (i + 1) % 5 === 0;
          labelText = i + 1;
        }
        if (!showLabel) return '';
        const x = padding.left + i * xStep;
        const isPeak = i === peakIdx && v > 0;
        return `<text class="rocket-day ${isPeak ? 'is-peak' : ''}" x="${x}" y="${padding.top + innerH + 18}">${labelText}</text>`;
      }).join('')}
      ...
      ${points.map(([x, y, v], i) => {
        if (v === 0) return '';
        const labels = options.labels;
        const tooltipLabel = labels && labels[i] ? labels[i] : `Dia ${i + 1}`;
        return `<circle class="rocket-day-marker" cx="${x}" cy="${y}" r="3.5">
          <title>${tooltipLabel}: ${v} fechamento(s)</title>
        </circle>`;
      }).join('')}
