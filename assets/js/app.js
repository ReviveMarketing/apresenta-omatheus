...
const App = {
  ...
  async handleOCRFiles(files) {
    ...
    for (const file of files) {
      ...
      const item = { id, file, blobUrl, status: 'queued', progress: 0, parsed: null, error: null };
      this.ocrItems.push(item);
      this.renderOCRQueue();

      // Processa um por vez (evita travar o navegador)
      try {
        ...
        item.parsed = OCR.parse(text);
        item.status = 'ready';
        this.renderOCRItem(item);
      } catch (err) {
        console.error(err);
        item.status = 'error';
        item.error = err.message || 'Erro ao processar';
        this.renderOCRItem(item);
      }
    }
  },
  ...
  renderOCRItem(item) {
    ...
    let html = `<img class="ocr-item__thumb" src="${item.blobUrl}" alt=""/><div class="ocr-item__body">`;
  ...
  saveOCRItem(id) {
    ...
    // Validação mínima
    if (!p.client || !p.date) {
      this.toast('Cliente e Data são obrigatórios. Use "Editar antes" para completar.', 'error');
      return;
    }
    const data = {
      type: p.type || 'digital',
      ...
      uf: p.uf || '',
      status: 'Fechado',
      origin: p.origin || 'META',
      neighborhood: '',
      notes: p.audited ? 'Auditado (importado via OCR)' : 'Importado via OCR'
    };
    ...
    this.renderOCRQueue();
    this.render();
    this.toast(`Fechamento de ${p.client} salvo`, 'success');
  },
  ...
  // Foguete: fechamentos por dia do mês
  renderRocket() {
    const { year, month } = this.state.period;
    const closings = Store.getClosingsForPeriod(year, month).filter(c => (c.status || 'Fechado') === 'Fechado');
    ...
    const avg = daysInMonth > 0 ? (total / daysInMonth).toFixed(1) : '0';
    ...
    Charts.renderRocket(document.getElementById('rocket-svg'), daily);
  },
