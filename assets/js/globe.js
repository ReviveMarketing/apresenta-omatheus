...
const Globe = {
  ...
  init(canvasId) {
    ...
    const animate = () => {
      ...
      // Atualizar posição dos labels HTML (projeção 3D → 2D)
      const inst = this.instances[canvasId];
      if (inst && inst.markerRefs && inst.markerRefs.length > 0) {
        const w = container.clientWidth;
        const h = container.clientHeight;
        const cameraDir = new THREE.Vector3();
        camera.getWorldDirection(cameraDir);
        inst.markerRefs.forEach(ref => {
          // Posição mundial do ponto (com a rotação do earth aplicada)
          const worldPos = ref.worldPos.clone().applyMatrix4(earth.matrixWorld);
          // Verifica se o ponto está virado pra câmera (não escondido atrás do globo)
          const toCam = new THREE.Vector3().subVectors(camera.position, worldPos).normalize();
          const normal = worldPos.clone().normalize();
          const dot = toCam.dot(normal);
          // Se ângulo > ~90° o ponto está atrás do globo
          if (dot < 0.05) {
            ref.labelEl.style.opacity = '0';
            return;
          }
          // Projeção
          ...
          ref.labelEl.style.left = x + 'px';
          ref.labelEl.style.top = y + 'px';
          ref.labelEl.style.opacity = Math.min(1, dot * 2.5);
        });
      }
  ...
  updateMarkers(canvasId, points) {
    const inst = this.instances[canvasId];
    if (!inst) return;
    const { markersGroup, container } = inst;
    ...
    // Limpar labels HTML antigos
    container.querySelectorAll('.globe-label').forEach(el => el.remove());
    ...
    points.forEach(p => {
      ...
      if (!grouped[key]) {
        grouped[key] = { lat: p.lat, lng: p.lng, count: 0, value: 0, types: { digital: 0, referral: 0 }, city: p.city, uf: p.uf };
      }
      grouped[key].count++;
    ...
    inst.markerRefs = []; // Para tracking dos labels
    ...
    Object.values(grouped).forEach((g, idx) => {
      ...
      label.innerHTML = `<span class="globe-label__city">${g.city || ''}</span><span class="globe-label__uf">${g.uf || ''}</span>`;
      container.appendChild(label);

      inst.markerRefs.push({
        // posição mundo do ponto (com beam acima do ponto):
        worldPos: pos.clone().multiplyScalar(1.05),
        labelEl: label,
        city: g.city,
        uf: g.uf
      });
